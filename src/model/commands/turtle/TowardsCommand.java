...
public class TowardsCommand extends TurtleCommand {
	...
	public double execute(double[] parameters, Turtle myTurtle, Controller view){
		double xdiff = parameters[0] - myTurtle.getDestination().getX();
		double ydiff = parameters[1] - myTurtle.getDestination().getY();
	...
	public Argument getReturnValue() {
		double xdiff = this.getParameterList().get(0) - this.getController().getTurtle().getDestination().getX();
		double ydiff = this.getParameterList().get(1) - this.getController().getTurtle().getDestination().getY();
		double tempd = Math.atan2(ydiff, xdiff);
